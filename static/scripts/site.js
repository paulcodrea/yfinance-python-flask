...
Site.prototype.GetQuote = function(){
	...
	$.ajax({
		url: "/quote?symbol=" + that.symbol + "&period=" + that.period + "&interval=" + that.interval,
		method: "GET",
		cache: false
	}).done(function(data) {
...
// Site.prototype.SubmitEvent = function(){
// 	// store the site context.
// 	this.event_1 = $("#event_1").val();
// 	this.event_2 = $("#event_2").val();
// 	this.GetQuote();
// };
...
Site.prototype.LoadChart = function(quote){
	...
	var that = this;
	$.ajax({
		// url: "/chart?symbol=" + quote.symbol ,
		url: "/history?symbol=" + that.symbol + "&period=" + that.period + "&interval=" + that.interval,
		method: "GET",
		cache: false
	}).done(function(data) {
...
Site.prototype.RenderChart = function(data, quote){
	...
	for(var i in data.Close){
		...
		var dateString = moment.unix(dt).format("DD/MM/YY HH:mm");
